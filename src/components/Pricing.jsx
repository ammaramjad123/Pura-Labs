...
import { Check, X, Star, Crown, Rocket, Award, Sparkles, Infinity, Users, Calendar, BarChart, Shield, MessageSquare, CheckCircle } from "lucide-react";
import { Link } from "react-router-dom";

const Pricing = () => {
  const [hoveredCard, setHoveredCard] = useState(null);
  const [openDropdown, setOpenDropdown] = useState(null);
  const [selectedOptions, setSelectedOptions] = useState([150, 150, 250]);
  ...
  const features = [
    ...
    { name: "Agents", basic: "1", plus: "2", pro: "3" },
    {
      name: "Languages",
      basic: "1",
      plus: "1",
  ...
  const renderFeatureRow = (feature, index) => {
    const key = index === 0 ? "basic" : index === 1 ? "plus" : index === 2 ? "pro" : null;
    if (!key) return null;
    ...
    return (
      <div className="flex items-center justify-between py-2 hover:bg-purple-50 rounded-lg px-2 transition">
        <div className="flex items-center gap-2">
          {typeof value === "boolean" && (value ? <Check className="w-5 h-5 text-green-600" /> : <X className="w-5 h-5 text-red-500" />)}
          <span className="text-sm text-gray-700">{feature.name}</span>
        </div>
        {typeof value !== "boolean" && <span className="text-sm font-semibold text-purple-600">{value}</span>}
      </div>
    );
  };
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-20">
        ...
        <div className="text-center mb-16">
          ...
          <h1 className="text-5xl md:text-7xl font-bold mb-6">
            <h2 className="text-3xl sm:text-5xl lg:text-6xl font-bold mb-6 font-heading text-neutral-950">Choose Your AI Agent</h2>
          </h1>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">Never miss another call or customer. Select the perfect plan for your business needs.</p>
        </div>

        {/* Cards - 3 columns with wider cards */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {plans.map((plan, index) => {
            const selected = selectedOptions[index];
            const selectedLabel = optionsMap[plan.name]?.find((o) => o.value === selected)?.label;
            ...
            return (
              ...
              >
                <div className={`relative bg-white rounded-3xl p-8 border flex flex-col h-full ${plan.popular ? "border-purple-400 shadow-2xl" : "border-gray-200"}`}>
                  {plan.popular && (
                    <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-purple-600 text-white px-4 py-1 rounded-full text-sm">Most Popular</div>
                  )}
                  <div>
                    <div className="text-center mb-6">
                      <div className={`w-14 h-14 mx-auto mb-4 rounded-xl bg-gradient-to-br ${plan.color} flex items-center justify-center text-white`}>{plan.icon}</div>
                    ...
                    <div className="relative mb-6">
                      ...
                      >
                        <span className="text-sm font-medium text-gray-700 pr-6">{selectedLabel}</span>
                      ...
                      {openDropdown === index && (
                        <div className="absolute left-0 top-full mt-2 w-full bg-white border border-gray-200 rounded-xl shadow-xl z-50 overflow-hidden">
                          {optionsMap[plan.name].map((opt, i) => (
                            ...
                            >
                              <span>{opt.label}</span>
                              <span className="text-purple-600 font-medium">£{pricingMap[plan.name][opt.value]}/mo</span>
                            </div>
                          ))}
                        </div>
                      )}
        ...
        <div className="text-center mt-8">
          <p className="text-xs text-gray-800">
            Setup fee applies. Monthly rolling contracts available.
          </p>
        </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-12 items-center">
                <div>
                  <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-purple-100 border border-purple-200 mb-6">
                    ...
                    <span className="text-sm font-semibold text-purple-600">Enterprise Grade</span>
                  </div>
                  <h2 className="text-3xl sm:text-4xl lg:text-4xl font-bold mb-6 leading-tight">
                    <span className="text-neutral-950 font-heading">Need Something More?</span>
                  </h2>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {[
                      ...
                      <div key={idx} className="flex items-center gap-3 text-gray-700">
                        <div className="w-10 h-10 rounded-xl bg-purple-100 flex items-center justify-center text-purple-600">{item.icon}</div>
                        <span className="font-medium">{item.label}</span>
                      </div>
                    ))}
                  </div>
                  <Link to="/contact">
                  <button className="mt-8 w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200 flex items-center justify-center gap-2 cursor-pointer">
                    Talk to Our Team
